...
const Body = () => {
  ...
  // we are passing the RestaurantCard to the higher order components such as withPromotedLabel and we get the new component called RestaurantCardDiscount (which contains discount on it)
  ...
  const fetchData = async () => {
    ...
    console.log("API Response JSON:", json); // Our broswer will throw an error says CORS error access denied means our broswer will block the access to fecth api from on eorigin to another origin
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        {/* ! Component is basically funciton, we use function while we have to do repitation task like same if there is repitation is code we have to use separate component */}
        {
          filteredRestaurant.map((restaurant) => (
            ...
            >
              {/* If the restaurant have discount the add a discount label to it */}
